...
/**
 * PermissionState — README §7-6 Viewer가 `/ops/batches`에 접근했을 때
 * 보여주는 403 화면. 문구는 README에 적힌 한국어 그대로다. 이 컴포넌트만
 * 렌더링되고 로그·Trigger·상세 노드는 DOM에 아예 없어야 한다는 것이
 * §7-6/§16-11의 요구사항 — 그 판단(무엇을 렌더링할지)은 호출부(배치
 * 운영 페이지)의 책임이고, 이 컴포넌트는 403 화면 자체만 그린다.
 */
  ...
  return (
    ...
    >
      ...
      <p className='wrap-anywhere mb-5 text-[13.5px] text-[color:var(--text-soft)]'>
        이 화면은 파이프라인 로그와 수동 실행을 포함하므로 Operator 권한이 있는
        계정만 열 수 있습니다. 현재 계정은 Viewer입니다.
      </p>
