...
- **전제** — 부록 A는 계약이고, 값이 채워졌는지와는 별개다. 최종 판정은 백엔드가 생성하는
  OpenAPI 스펙이며 `docs/api-spec.json`은 **아직 갱신되지 않았다**(신규 필드 0건).
...
| # | 항목 | 차단 요인 | 난이도 |
|---|---|---|---|
| §2 | 반복 쿼리 파라미터 직렬화 | 없음. **B-3의 선행 조건** | 낮음 |
| §3 | B-3 계층형 테마·아카이브 검색 | 백엔드 엔드포인트 미구현 | 높음 |
| §4 | A-7 잔여 — 공개 아카이브 `status=FAILED` 제거 | 없음 (B-3 영역이라 함께 미뤘을 뿐) | 낮음 |
| §5 | OpenAPI 갱신 후 DTO 재검증 | 백엔드의 `api-spec.json` 갱신 대기 | 낮음 |
...
- §4는 §3과 파일이 겹치므로(둘 다 아카이브 검색 화면) §3 착수 직전에 끝내 두면 충돌이 없다.
  다만 §4는 **지금도 사용자에게 보이는 문제**이므로 §3을 오래 미룰 거라면 먼저 떼어내도 좋다.
- §5는 백엔드가 스펙을 갱신한 뒤 언제든.

### 작업 시작 전 확인

- 2026-08-14 변경분은 **커밋되지 않은 상태**일 수 있다. `git status`로 먼저 확인한다.
- 알려진 사전 존재 실패 2건은 이번 작업의 산물이 아니다. 새로 만든 것과 혼동하지 말 것.
  - `pnpm exec playwright test` — `routing.spec.ts`의 "Back from a cluster detail page restores
    the selected tab and scroll position" 1건 (스크롤 복원 타이밍)
  - `pnpm run knip` — `@biomejs/biome` devDependency, `biome` 바이너리 2건
    (`AGENTS.md`에 기존 항목으로 명시돼 있다)

---

## 2. 반복 쿼리 파라미터 직렬화 — **B-3 선행 조건**

부록 A-4-3이 **B-3 착수 전에 확인하라**고 명시한 항목이다. 확인 결과 **현재 클라이언트는
반복 파라미터를 만들 수 없다.**
...
## 3. B-3 계층형 테마 · 아카이브 검색

**차단 상태** — `GET /stock/api/pages/archive/themes`가 없고, `GET /stock/api/pages/archive`는
여전히 `fromDate` · `toDate` · `status` · `page` · `size`만 받는다. **실호출 검증은 백엔드
완료 후에만 가능하다.** UI·URL 상태·쿼리 키까지는 fixture로 선구현할 수 있다.
...
## 4. A-7 잔여 — 공개 아카이브 `status=FAILED` 제거

**지금도 보이는 문제다.** 서버는 공개 조회(`/archive`, `/daily`, `/navigation`)에서 `FAILED`
버전을 완전히 제외한다(A-1-9). 그런데 검색 화면에는 아직 `FAILED` 옵션이 남아 있어 **선택하면
항상 빈 결과가 나온다.**
...
## 5. OpenAPI 갱신 후 DTO 재검증

`docs/api-spec.json`이 갱신되면 아래를 대조한다. **부록 A와 스펙이 어긋나면 스펙이 이긴다**(A-8).
...
B-2와 B-4는 **응답 형태는 최종본이지만 값이 아직 고정값**이다(A-0). 백엔드가 읽기 경로를
연결하면 **코드 변경 없이 값만 채워져야 한다.** 그때 확인할 것:
